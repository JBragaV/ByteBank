...
internal class ByteBankAtendimento
{
    ...
    private void ExportarContas()
    {
        ...
        else
        {
            ...
            try
            {
                XmlSerializer xml = new XmlSerializer(typeof(List<ContaCorrente>));

                FileStream fs = new FileStream("contas.json", FileMode.Create);
                FileStream fsXML = new FileStream("contas.xml", FileMode.Create);
                using (StreamWriter sw = new StreamWriter(fs))
                {
                    sw.WriteLine(json);
                    sw.Flush();
                }
                using (StreamWriter sw = new StreamWriter(fsXML))
                {
                    xml.Serialize(sw, _listaDeContas);
                }

                    Console.WriteLine(@"Arquivo salvo corretamente!!!");
            }
            catch (Exception ex)
            {
